...
const JUNIOR_SUBJECTS = [
  { name: 'English Written', max: 50 },
  { name: 'English Oral', max: 50 },
  { name: 'Urdu Written', max: 50 },
  { name: 'Urdu Oral', max: 50 },
  { name: 'Maths Written', max: 50 },
  { name: 'Maths Oral', max: 50 },
  { name: 'Al Quran + Duaien', max: 50 },
  { name: 'Color & Craft', max: 50 },
];

const SENIOR_SUBJECTS = [
  { name: 'Math', max: 100 },
  { name: 'Urdu A (W)', max: 75 },
  { name: 'Urdu B (O)', max: 75 },
  { name: 'Computer / Biology', max: 50 },
  { name: 'Social Study / Pak Studies', max: 50 },
  { name: 'English A (W)', max: 75 },
  { name: 'English B (O)', max: 75 },
  { name: 'Al Quran / Duain', max: 50 },
  { name: 'Islamiyat', max: 50 },
  { name: 'Science / Physics', max: 50 },
  { name: 'Chemistry / Education', max: 50 },
  { name: 'G Knowledge / Islamiyat (E)', max: 50 },
  { name: 'Rhymes / Drawing', max: 50 },
];

type IntimationType = 'custom' | 'junior' | 'senior';
...
export default function ExamsPage() {
  ...
  const [selectedIntimation, setSelectedIntimation] = useState<IntimationType>('custom');
  ...
  const handleIntimationChange = (type: IntimationType) => {
    setSelectedIntimation(type);
    if (type === 'junior') {
      setSubjects(JUNIOR_SUBJECTS.map(s => s.name));
      const maxObj: Record<string, number> = {};
      JUNIOR_SUBJECTS.forEach(s => maxObj[s.name] = s.max);
      setMaxMarksPerSubject(maxObj);
    } else if (type === 'senior') {
      setSubjects(SENIOR_SUBJECTS.map(s => s.name));
      const maxObj: Record<string, number> = {};
      SENIOR_SUBJECTS.forEach(s => maxObj[s.name] = s.max);
      setMaxMarksPerSubject(maxObj);
    }
  };
  ...
  const handleClassSelect = (val: string) => {
    ...
    if (v.includes('play') || v.includes('nursery') || v.includes('prep')) {
      handleIntimationChange('junior');
    } else if (v.includes('grade') || v.includes('class') || v.match(/\d/)) {
      handleIntimationChange('senior');
    } else {
      handleIntimationChange('custom');
    }
  };
            ...
            <CardContent className="p-4 space-y-4">
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <label className="text-xs font-semibold text-slate-600 block">Subjects Configuration</label>
                  <select
                    value={selectedIntimation}
                    onChange={(e) => handleIntimationChange(e.target.value as IntimationType)}
                    className="h-8 rounded-lg border border-slate-200 px-2 text-xs font-semibold focus:outline-none focus:ring-2 focus:ring-blue-500 bg-slate-50"
                  >
                    <option value="custom">Custom Subjects</option>
                    <option value="junior">Junior Intimation (Fixed 450 Marks)</option>
                    <option value="senior">Senior Intimation (Customizable)</option>
                  </select>
                </div>
                ...
                <div className="flex flex-wrap gap-2">
                  {subjects.map((sub) => (
                    <div key={sub} className="flex items-center gap-1 bg-blue-50 border border-blue-200 rounded-lg px-2 py-1">
                      ...
                      <span className="text-[10px] text-blue-400 bg-blue-100 px-1 rounded">({maxMarksPerSubject[sub] || 100})</span>
                      {selectedIntimation !== 'junior' && (
                        <button onClick={() => removeSubject(sub)} className="text-blue-400 hover:text-red-500 ml-1">
                          <X className="h-3 w-3" />
                        </button>
                      )}
                    </div>
                ...
                {selectedIntimation !== 'junior' && (
                  <div className="flex gap-2">
                    <Input
                      ...
                      className="border-slate-200 text-sm h-9 max-w-sm"
                    />
